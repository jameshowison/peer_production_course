...
skills_topics <- tribble(
  ...
  "Release management", "[Git exercises](docs/additional_git_exercises.html) and github releases, git tag",
...
) %>% mutate(topic = "Skills") %>%  rownames_to_column()
...
introduction_topics <- tibble(introduction_topic = c("Syllabus Review", "Paper Plane conceptual exercise"), topic = "Introduction") %>%  rownames_to_column()
...
no_meet <- meets_schedule %>% filter(topic == "No Meeting")
insights <- meets_schedule %>% filter(topic == "Insights") %>% rownames_to_column() %>% 
  left_join(insight_topics) %>% 
  mutate(class_name = insight_topic) %>% 
  select(-insight_topic)

skills <- meets_schedule %>% filter(topic == "Skills") %>% rownames_to_column() %>% 
  left_join(skills_topics) %>% 
  mutate(class_name = skill_topic) %>% 
  select(-skill_topic)



schedule %>% 
  mutate(monday_class = mon_schedule, wed_class = wed_schedule)
...
meetings %>%
  mutate(additional_class = ,
         class_day = class_day %>% format("%a %b %d"),
         meeting_dates = str_c(format(class_day, "%a %b %d"), format(class_day + days(2), "%a %b %d"), sep = " & ")) %>%
#   mutate(class_day = cell_spec(class_day, "html", color = ifelse(meets == TRUE, "green", "red"))) %>%
  select(-meets, -additional_class) %>%
